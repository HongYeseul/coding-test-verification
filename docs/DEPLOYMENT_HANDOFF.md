...
여섯 버전을 `supabase_migrations.schema_migrations`에도 등록했습니다. 기존 마이그레이션을 재실행하지 않고 새 마이그레이션부터 적용합니다.
...
- 플랫폼 계정 없이 JPG·PNG·WebP 원본을 20MB까지 선택합니다. 브라우저에서 긴 변 1,920px 이하, 약 500KB 목표로 압축한 후 업로드합니다. 저장 한도는 6MB이며 작은 원본이 더 작으면 유지합니다. 제목은 선택이고 사진 속 문제명·날짜 자동 추출은 하지 않습니다.
