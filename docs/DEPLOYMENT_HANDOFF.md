...
- 플랫폼 계정 없이 JPG·PNG·WebP 원본을 20MB까지 선택하거나 캡처를 복사해 붙여넣습니다. 긴 변 최대 1,440px, 120KB 목표로 압축하며 필요하면 1,200px·1,024px까지 줄입니다. 미리보기로 글자를 확인한 뒤 등록하며 새 파일의 저장 상한은 브라우저와 Storage 버킷 모두 300KB입니다. 기존 사진과 검수·취소는 유지합니다. 제목은 선택이고 사진 속 문제명·날짜 자동 추출은 하지 않습니다.
