...
3인자 `create_group`을 쓰는 화면을 배포한 뒤 `20260913010000`으로 구버전 2인자 함수를 지웠습니다. 지금은 `create_group(text, text, boolean)` 하나만 남아 있습니다.

배포 전에 지우면 구버전 화면의 그룹 만들기가 막히므로, 두 시그니처를 잠시 함께 두었다가 배포 후에 정리하는 순서를 지켰습니다. 기본값 없는 3인자라 인자 수로 구분되어 모호하지 않았습니다. `20260909030000_drop_list_group_directory.sql`과 같은 규칙이며, 앞으로 RPC 시그니처를 바꿀 때도 이 순서를 지킵니다.

`20260911000000`의 백필은 기존 그룹을 모두 코딩 테스트 스터디로 옮겼습니다. 적용 시점에 그룹 1개·활성 멤버 7명·인증 17건이 있었고 기록은 그대로 유지됐습니다.

`20260913020000_proof_tags.sql`은 `proofs.tags`를 더합니다. CHECK에 하위 질의를 쓸 수 없어 `private.valid_proof_tags()` IMMUTABLE 함수로 검사합니다. 컬럼 추가라 구버전 코드와 무관하지만 새 코드가 조회하므로 배포보다 먼저 적용했습니다.
