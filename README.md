...
- **GitHub 저장소를 연결하면**: 카드로 도장을 찍을 때 풀이 코드와 README가 고른 저장소에 커밋 하나로 올라갑니다. README에는 제목·플랫폼·문제 링크·태그만 적고 문제 설명은 옮기지 않습니다. 같은 GitHub 로그인에 `public_repo` 권한을 더해 받으므로 공개 저장소만 고를 수 있고, 토큰은 확장에만 두고 서버로 보내지 않습니다. 올리다 실패해도 도장은 남습니다.
- **설치하거나 새 버전으로 바꾸면**: 이미 열려 있던 문제 탭에도 감지 스크립트를 넣어, 새로고침하지 않아도 카드가 뜹니다. 크롬은 manifest의 콘텐츠 스크립트를 설치한 뒤에 불러온 페이지에만 넣기 때문입니다. `scripting` 권한은 이 일에만 씁니다.
...
전부는 `/releases`가 집니다. 카드의 ‘지난 기록’이 여기로 옵니다 — GitHub 릴리스로 보내면 거기엔 확장 이야기밖에 없어 웹 기록을 볼 데가 없습니다. 이 페이지도 로그인 없이 봅니다. 확장의 더 지난 버전과 내려받을 파일만 GitHub 릴리스로 넘깁니다.

내용은 `src/lib/release-notes.ts`의 `RELEASES` 한 곳에 있고 최신이 위입니다. 카드는 `latestReleases()`로 면마다 하나씩 고르고, 두 화면이 어긋나지 않도록 릴리스 한 건을 그리는 일은 `ReleaseEntry`가 함께 씁니다. 확장 버전은 `extension/manifest.json`에서 직접 읽습니다 — 손으로 옮겨 적으면 릴리스를 낸 뒤 여기만 옛 번호로 남습니다. 지난 확장 릴리스를 더할 때만 그 항목에 `version`을 적고, **가장 최근 것은 비워 둡니다.** 받기 단추도 그 기준으로 최신 확장에만 붙습니다. 웹은 번호가 없어 배포한 날짜로 가릅니다.
...
| **저장소 · 올리기** | 확장이 풀이를 커밋하는 GitHub 저장소와 그 일. ‘레포’·‘푸시’라고 쓰지 않습니다 |
...
목록 위에는 스터디 수·멤버 수·누적 인증 횟수를 한 줄로 요약합니다. 멤버 수는 사람 수라 여러 스터디에 든 사람도 한 번만 셉니다 — 그룹별 인원을 그냥 더하면 두 번 세어집니다. 인증은 서비스를 연 뒤 쌓인 전체이며 비공개 스터디까지 넣습니다. 멤버 수가 이미 전체를 세고 있어 한쪽만 공개 스터디로 좁히면 같은 줄에서 기준이 엇갈립니다. 합계라 어느 스터디의 것인지는 드러나지 않습니다. 줄 안의 ‘이번 주 N번’은 그대로 그 스터디의 이번 주 기록입니다. 공개 스터디의 줄에는 월요일부터 일요일까지 일곱 칸을 두어 인증이 있던 날을 채웁니다. 그룹 화면의 주간 인증 매트릭스를 한 줄로 줄인 것이라 로그인 전후로 같은 모양을 봅니다. 아직 오지 않은 날은 흐리게 둬서 이번 주의 어디까지 왔는지 함께 보여줍니다.
