...
제목을 비워두고 등록하면 목록에 링크가 그대로 보입니다. 그룹의 `OWNER`·`REVIEWER`는 목록에서 ‘제목 넣기·제목 수정’을 열어 제목을 채울 수 있습니다. 목록이 같은 링크를 한 문제로 묶으므로 저장하면 그룹 안에서 같은 링크를 남긴 기록이 모두 같은 제목을 갖고, 풀이 기록 목록의 제목도 함께 바뀝니다. 제목을 비우면 다시 링크가 보입니다. 링크 자체는 여전히 고칠 수 없습니다.

`proofs`의 UPDATE 권한은 `problem_title` 한 컬럼만 열어 두었습니다. RLS는 행 단위라 컬럼을 막지 못하므로 `profiles`와 같은 방식으로 컬럼 단위 권한을 씁니다. 검수 권한은 서버 액션과 RLS 정책에서 모두 `ACTIVE` 멤버의 `OWNER`·`REVIEWER` 여부로 확인합니다.
