...
주간 현황은 멤버를 행, 월요일부터 일요일까지를 열로 놓은 표입니다. 멤버는 선택한 주의 승인 건수가 많은 순으로 놓고, 같으면 누적 승인이 많은 순, 그다음 닉네임순입니다. 멤버 열 머리글에 `· 이번 주 승인순`이라고 적고 마우스를 올리면 동점 규칙까지 설명합니다. 주를 옮기면 그 주의 승인 건수로 순서를 다시 계산합니다. 기록이 있는 칸은 검수 대기 `◷`, 승인 `✓`, 반려 `×` 중 하나를 보여주고 두 건 이상이면 건수를 덧붙입니다. 칸을 누르면 그 멤버와 날짜로 좁힌 풀이 기록으로 이동합니다. 지나지 않은 날은 `–`, 기록 없는 날은 `·`입니다.
