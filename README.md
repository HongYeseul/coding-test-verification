...
- 그룹의 최근 활동 — 도장·착석·퇴근·응원·검수를 시간순으로 모은 오른쪽 칸
...
오른쪽 칸에는 어느 그룹에나 ‘최근 활동’이 있습니다. 도장판이 무엇이 쌓였는지를 보여준다면 여기는 방금 무슨 일이 있었는지를 보여줍니다. 도장·착석·퇴근·응원·검수를 시간순으로 한 줄씩 적고, 반려 이유는 요약하지 않고 그대로 인용합니다. 오늘 일은 시각으로, 지난 일은 날짜로 적습니다 — ‘3분 전’ 같은 표현은 서버에서 그린 순간 굳어 버립니다. 서로 독려하는 서비스라 남이 움직이는 것이 보여야 해서 둔 자리입니다. 코딩 테스트 스터디는 그 위에 ‘우리 그룹이 푼 문제’가 함께 놓입니다.
