...
사진을 올리면 검수 대기 풀이 기록이 생성됩니다. 플랫폼 계정·문제 URL은 필요하지 않고 제목은 선택입니다. JPG·PNG·WebP 사진을 6MB까지 올릴 수 있습니다. 사진의 문제명·풀이 날짜를 자동 추출하지 않으며 등록 시각을 기록합니다. 그룹의 `OWNER` 또는 `REVIEWER`가 사진을 확인합니다.
