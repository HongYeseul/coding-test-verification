...
- **코딩 테스트 스터디**: 켜면 문제 링크 입력과 ‘우리 그룹이 푼 문제’ 목록을 쓰고, 문구도 ‘풀이 인증하기’가 됩니다. 끄면 ‘인증하기’로 바뀌고 오른쪽 문제 목록 칸이 사라져 한 칸으로 놓입니다.
...
폭 1024px 안에 상단바와 그룹 헤더를 두고, 그 아래를 넓은 화면에서 두 칸으로 나눕니다. 왼쪽에 주간 현황과 풀이 기록, 오른쪽에 ‘우리 그룹이 푼 문제’를 두며 오른쪽 칸은 스크롤을 따라옵니다. 1024px보다 좁으면 세로로 쌓입니다. 그림자 없이 1px 테두리와 라운드만 씁니다. 색과 모서리는 토큰 한 벌로 모아 `light-dark()`로 라이트·다크를 함께 처리합니다. 값은 [디자인 시스템](#디자인-시스템)에 정리해 두었습니다. 본문은 15px이고 목록에서 가장 먼저 읽는 풀이 제목은 17px입니다. 보조 설명은 13px, 집계 같은 메타 정보는 12px을 씁니다. 간격은 모두 4px 배수를 씁니다.
...
기록이 있는 칸은 승인이면 도장을 찍고, 검수 대기 `◷`와 반려 `×`는 둥근 네모 테두리로 둡니다. 두 건 이상이면 칸 어깨에 건수를 답니다. 승인만 형태가 달라서 찍힌 칸이 곧 그 주의 성과로 읽히고, 색을 구분하지 못해도 나뉩니다. 오늘 열은 배경으로 표시합니다. 칸을 누르면 그 멤버와 날짜로 좁힌 풀이 기록으로 이동합니다. 지나지 않은 날은 `–`, 기록 없는 날은 `·`이며 둘 다 흐리게 둡니다.
...
스펙 시트는 `docs/design/`에 있습니다. 값을 바꿨으면 `python3 docs/design/build-sheets.py`로 다시 만듭니다.
...
| 토큰 | 라이트 | 다크 | 쓰임 |
|---|---|---|---|
| `--canvas` | `#ffffff` | `#14181d` | 바탕 |
| `--ink` | `#1b2027` | `#e9edf2` | 본문 |
| `--sub` | `#69737e` | `#98a3af` | 보조 문구 |
| `--line` | `#e3e7ec` | `#2f353d` | 경계선 |
| `--soft` | `#f4f6fa` | `#1c2128` | 눌린 면, 호버 |
| `--brand` | `#1d5091` | `#8fc0f2` | 도장, 오늘 열, 포커스 링 |
| `--brand-soft` | `#e9f0fa` | `#1b2e47` | 브랜드 배경면 |
...
`--brand`는 바탕 위에 얹는 색(글자·도장·선)이고 `--primary`는 채우는 면(버튼 배경)입니다. 대비 기준이 다르므로 값도 다릅니다. 중성색은 파랑 쪽으로 치우쳐 있습니다 — 브랜드가 파랑인데 회색이 초록기를 띠면 때가 낀 것처럼 보입니다.
