...
제목을 비워두고 등록하면 목록에 링크가 그대로 보입니다. 그룹의 `OWNER`·`REVIEWER`는 목록에서 ‘제목 넣기·제목 수정’을 열어 그룹이 쓸 제목을 정할 수 있습니다. 이 제목은 `group_problem_titles`에 그룹별로 따로 보관하며 목록에서만 기록에 적힌 제목보다 앞섭니다. 각 멤버가 자기 기록에 적은 제목과 풀이 기록 목록은 그대로 둡니다. 제목을 비우면 행을 지워 기록에 적힌 제목으로 되돌아갑니다. 링크 자체는 여전히 고칠 수 없습니다.
