...
주간 현황은 멤버를 행, 월요일부터 일요일까지를 열로 놓은 표입니다. 기록이 있는 칸은 검수 대기 `◷`, 승인 `✓`, 반려 `×` 중 하나를 보여주고 두 건 이상이면 건수를 덧붙입니다. 칸을 누르면 그 멤버와 날짜로 좁힌 풀이 기록으로 이동합니다. 지나지 않은 날은 `–`, 기록 없는 날은 `·`입니다.
...
닉네임은 같은 그룹 안에서 겹칠 수 있습니다. 대신 프로필에 로그인한 GitHub 아이디를 함께 보관해 이름을 바꿔도 누구인지 확인할 수 있게 합니다. 아이디는 소유자의 가입 승인·검수자 지정 화면과 풀이 상세 모달의 작성자 옆에 나오고, 주간 현황에서는 멤버 이름 칸에 마우스를 올리면 소개와 함께 보입니다.
