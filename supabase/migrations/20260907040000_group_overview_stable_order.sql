-- 보여줄 순서는 화면이 정하므로 함수는 응답 배열의 순서만 고정합니다.
-- 정렬 기준은 표시 규칙이라 바꿀 때마다 마이그레이션이 필요하지 않도록
-- src/components/group-overview.tsx의 sortedMembers로 옮겼습니다.
-- 여기서는 같은 요청에 항상 같은 배열이 나오도록 user_id로만 정렬합니다.
begin;
