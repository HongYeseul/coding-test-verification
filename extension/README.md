...
팝업 아래 **GitHub 저장소**를 펼쳐 연결하고 저장소를 한 번 고르면, 카드로 도장을 찍을 때
풀이가 그 저장소에도 올라갑니다. 누르는 것은 여전히 도장 찍기 하나이고, 연결하지 않으면
지금과 같습니다. 올라가는 것은 카드로 찍은 풀이뿐입니다. 화면 캡처와 웹에서 붙여넣은 코드는
올리지 않습니다.

![저장소에 올린 뒤의 결과 카드](screenshots/card-repo.png)

한 문제가 한 폴더이고, 코드와 README가 커밋 하나로 들어갑니다. 원격 브랜치를 바로 옮기므로
push 단계가 따로 없습니다.

```
프로그래머스/42576. 완주하지 못한 선수/solution.py
프로그래머스/42576. 완주하지 못한 선수/README.md
NeetCode/duplicate-integer/solution.py
NeetCode/duplicate-integer/README.md
```

README에는 제목·플랫폼·문제 링크·태그만 적습니다. **문제 설명은 옮기지 않습니다.** 플랫폼
콘텐츠라 읽지도 않고, 저장소가 공개라 더 그렇습니다. 백준허브와 다른 점이 이것입니다. 같은
문제를 다시 풀면 같은 파일을 덮어써 이력으로 남고, 내용이 같으면 커밋하지 않습니다. 파일
확장자는 제출한 언어로 정하고, 모르는 언어는 코드를 잃지 않도록 `solution.txt`로 둡니다.

도장과 올리기는 따로 갑니다. 올리다 실패해도 도장은 남고, 결과 카드에서 다시 올릴 수
있습니다. 올리는 동안과 실패했을 때는 카드가 사라지지 않습니다. 올라갔는지 보기 전에 닫히면
확인할 길이 없기 때문입니다.

**권한.** 연결하면 같은 GitHub 로그인을 `public_repo` 권한을 더해 한 번 더 거칩니다. 웹 로그인은
그대로입니다. OAuth 앱 권한은 저장소 하나로 좁힐 수 없어 **공개 저장소 전체에 쓰는 권한**이고,
그래서 비공개 저장소는 고를 수 없습니다. 비공개까지 쓰려면 모든 비공개 저장소를 여는 `repo`를
받아야 합니다. 저장소 하나로 좁히는 길은 GitHub App을 따로 등록하는 것인데, 코드를 토큰으로
바꾸는 데 client secret이 필요해 서버가 끼어야 합니다. 지금은 확장만 고치는 쪽을 골랐습니다.

**토큰.** Supabase는 GitHub 토큰을 로그인 응답에 한 번 실어 줄 뿐 보관하지 않으므로 확장이
`chrome.storage.local`에 둡니다. 서버로는 보내지 않고 `api.github.com`만 부릅니다. 이 주소는
어느 오리진에나 CORS를 열어 두어 사이트 권한을 새로 받지 않습니다. GitHub가 토큰을 거절하면
토큰만 버리고 고른 저장소는 남겨, 같은 계정으로 다시 연결하면 그대로 이어 씁니다.
‘저장소 연결 끊기’와 ‘연결 끊기’는 확장이 가진 토큰을 버리는 데까지입니다. GitHub에서 권한까지
거두려면 [Authorized OAuth Apps](https://github.com/settings/applications)에서 지웁니다.
토큰을 폐기하는 API는 client secret이 있어야 해서 확장이 부를 수 없습니다.

**로그인 창은 background가 띄웁니다.** 팝업은 포커스를 잃으면 닫혀, 팝업에서 창을 띄우면 창이
돌아오기 전에 받을 곳이 사라집니다. 닫혀도 연결은 끝나고, 다시 열면 저장소를 고르는 칸이
펼쳐져 있습니다.
...
- 프로그래머스에서는 **채점 결과 모달의 제목** (버튼을 띄울지 정하는 데만 쓰고 서버로
  보내지 않습니다), **본인이 제출한 코드**, 그 코드의 **언어 값**(에디터에 붙은
  `data-language`. 저장소에 올릴 파일 확장자만 정합니다)
- NeetCode에서는 **제출 요청에 실려 나가는 본인 코드와 언어**, **정답 여부**, 그리고 탭 제목

문제 설명·입출력 예시·힌트·모범답안 같은 플랫폼 콘텐츠는 읽지 않습니다. GitHub 저장소에도
올리지 않습니다.
...
| `popup.js` | 스터디 목록 조회, 업로드, 등록 요청, 저장소 고르기 |
| `background.js` | 콘텐츠 스크립트 대신 서버에 등록 요청 (페이지 오리진에서는 못 보냄), 저장소 연결과 올리기 |
| `github.js` | 저장소 연결·목록·만들기와 커밋. `api.github.com`만 부릅니다 |
| `solution-files.js` | 저장소에 올릴 경로와 README를 짓습니다. 크롬 API가 없어 node 테스트가 그대로 부릅니다 |
...
`screenshots/`의 여섯 장은 실제 `overlay.css`·`popup.css`·`seal.js`로 렌더한 것입니다.
카드는 `programmers.js`가 만드는 DOM과 같은 모양으로 다시 짜서 찍었고, 팝업은
`popup.html`에서 상태만 열어 찍었습니다. 담긴 문제·태그·스터디·저장소 이름은 예시입니다 —
공개 저장소라 실제 스터디원의 기록을 넣지 않았습니다. `popup-form.png`와 `card-repo.png`는
2026-09-24에 `card.js`로 그린 카드와 크롬 API를 흉내 낸 `popup.html`을 헤드리스 크롬으로 2배
찍었습니다.
...
형식은 같습니다. GitHub 저장소에도 올리려면 `solution-files.js`의 `PLATFORMS`에 문제
주소를 폴더로 바꾸는 규칙을 더합니다. 없으면 도장은 찍히고 올리기만 실패합니다.
