...
연결하면 스터디를 고르고 제목·태그를 적어 지금 보이는 화면을 캡처해 올립니다. 탭 주소가
지원 플랫폼이면 문제 링크가 자동으로 채워집니다.
...
화면 캡처는 `activeTab`만 쓰므로 사이트 권한이 필요 없습니다. 정답 감지는 프로그래머스
페이지를 봐야 해서 그 사이트 권한이 필요하고, 설치할 때 그 경고가 뜹니다.

### 깨질 수 있는 부분

정답 감지의 선택자는 프로그래머스 UI가 바뀌면 깨집니다. 깨져도 **카드가 안 뜰 뿐** 다른
건 멀쩡하고, 그때는 아이콘을 눌러 화면 캡처로 등록하면 됩니다.

코드는 에디터와 동기화된 `textarea`에서만 읽습니다. CodeMirror는 화면에 보이는 줄만
DOM에 두기 때문에 거기서 긁으면 코드가 잘립니다. **잘린 코드를 올리느니 카드를 안 띄우는
쪽**을 골랐습니다.
